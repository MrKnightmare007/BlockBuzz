...
async function getOrCreateCoreApplication({
  client,
  context,
  spinner
}) {
  const {
    prompt,
    cliConfig
  } = context, organizationId = cliConfig && "__experimental_coreAppConfiguration" in cliConfig && cliConfig.__experimental_coreAppConfiguration?.organizationId;
  spinner.succeed();
  const selectedApp = await selectExistingApplication({
    client,
    prompt,
...
async function getOrCreateCoreAppFromConfig({
  ...
  return output.print("The appId provided in your configuration is not recognized."), output.print("Checking existing applications..."), getOrCreateCoreApplication({
    client,
    context,
    spinner
  });
...
exports.getOrCreateCoreApplication = getOrCreateCoreApplication;
